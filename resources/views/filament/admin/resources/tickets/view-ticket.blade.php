<x-filament-panels::page>
    ...
    <div class="ticket-wrap">
        ...
        {{-- HEADER --}}
        <div class="ticket-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:40px; gap:20px; flex-wrap:wrap;">
                ...
                <table class="ticket-table">
                    ...
                    <tbody>
                        @foreach($record->items as $item)
                            @php
                                $precioConfig = $item->prenda?->precios()
                                ...
                                if ($record->tipo === 'encargo_express') {
                                    ...
                                    if ($precioExpress > 0) {
                                        $explicacion = $cantidad . ' × $' . number_format($precioExpress, 2) . ' = $' . number_format($item->subtotal, 2);
                                    } else {
                                        $explicacion = 'El ticket está en modo express, pero no se encontró precio express actual; se muestra el subtotal guardado.';
                                    }
                                } else {
                                    if ($piezasPorPaquete > 0 && $precioPaquete > 0) {
                                        ...
                                        if ($paquetes > 0) {
                                            ...
                                            $partes[] = $paquetes . ' paquete(s) de ' . $piezasPorPaquete . ' por $' . number_format($precioPaquete, 2);
                                            ...
                                            $explicacion = $cantidad . ' × $' . number_format($precioNormal, 2) . ' = $' . number_format($item->subtotal, 2);
                                        }
                                    } else {
                                        $explicacion = $cantidad . ' × $' . number_format($precioNormal, 2) . ' = $' . number_format($item->subtotal, 2);
                                    }
